...
const petURL = __dirname+'/src/files/pets.txt';
const userURL = __dirname+'/src/files/users.txt';
...
class Adopcion{
    ...
    async getUserById(id){
        try{
            console.log(id);
            let data = await fs.promises.readFile("./src/files/users.txt",'utf-8');
            let users = JSON.parse(data);
